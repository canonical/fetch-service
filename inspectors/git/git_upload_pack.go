...
func (UploadPackInspector) ID() string {
...
func (ins *UploadPackInspector) InspectRequest(a *metadata.Artefact) error {
	...
	a.Request.Body, err = newUploadPackRequestHandler(ins, a.Request, &client_msgs)
	if err != nil {
	...
	case "fetch":
		...
		if !isShallow {
			a.Reject(ins, "fetch is only allowed with depth 1").Annotate(notes)
			return nil
		} else if numWants > 1 {
			a.Reject(ins, "fetch is only allowed on a single ref").Annotate(notes)
			return nil
		}
	default:
		a.Reject(ins, "only ls-refs and fetch commands are allowed").Annotate(notes)
		return nil
	}

	a.Hold(ins, "valid URL for git upload-pack").Annotate(notes)
	return nil
}
...
func (ins *UploadPackInspector) InspectArtefact(f ReadAtSeeker, a *metadata.Artefact) error {
	...
	command, ok := a.RequestAnnotation(ins, "command") // the upload-pack request command
	if !ok {
	...
	case "ls-refs":
		if !a.MimeType.Is("text/plain") {
			a.Reject(ins, "bad data type for ls-refs response")
			return nil
		}
		...
		if err != nil {
			return fmt.Errorf("cannot read ls-refs response: %w", err)
		}
		...
		a.Approve(ins, "git ls-refs response decoded").Annotate(notes)

	case "fetch":
		if !a.MimeType.Is("application/octet-stream") {
			a.Reject(ins, "bad data type for fetch response")
			return nil
		}

		a.Metadata.Type = mimetypes.GitUploadPackFetch

		if numWants, ok := a.RequestAnnotation(ins, "num-wants"); !ok || numWants != 1 {
			notes.Add("num-wants", numWants)
			a.Reject(ins, "fetch is allowed only on a single ref").Annotate(notes)
			return nil
		}

		buf := make([]byte, 512)
		if _, err := f.Read(buf); err != nil {
			return errors.New("cannot read fetch response")
		}

		msgs, err := decodeGitProtocol(buf)
		if err != nil {
			return errors.New("error decoding git protocol")
		}
		...
		notes.Add("server-response", server_msgs)
		a.Comment(ins, "git fetch response is valid but content is unknown").Annotate(notes)

	default:
		a.Reject(ins, "only ls-refs and fetch commands are supported").Annotate(notes)
	}
...
func newUploadPackRequestHandler(ins metadata.Identifiable, req *http.Request, client_msgs *[]string) (*uploadPackRequestHandler, error) {
