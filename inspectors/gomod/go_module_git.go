...
func (ins *GoModuleGitInspector) InspectRequest(a *metadata.Artefact) error {
	...
	command, ok := a.RequestStringAnnotation(&git.UploadPackInspector{}, "command")
	if !ok || command != "fetch" {
		return nil // we don't recognize this request
	}
	...
	a.Hold(ins, "valid URL for go module download")
	return nil
}
...
func (ins *GoModuleGitInspector) InspectArtefact(f ReadAtSeeker, a *metadata.Artefact) error {
	...
	command, ok := a.RequestStringAnnotation(&git.UploadPackInspector{}, "command") // the upload-pack request command
	if !ok {
	...
	if !a.MimeType.Is("application/octet-stream") {
		a.Reject(ins, "bad data type for go module")
		return nil
	}

	// Read wants information from the git inspector annotation
	w, has_wants := a.RequestAnnotation(&git.UploadPackInspector{}, "wants")
	wr, has_want_refs := a.RequestAnnotation(&git.UploadPackInspector{}, "want-refs")
	...
	isShallow, ok := a.RequestBoolAnnotation(&git.UploadPackInspector{}, "is-shallow")
	if !ok {
	...
	if has_wants {
		...
		a.Reject(ins, "want-refs handling not implemented yet").Annotate(notes)
		return nil
	}

	goModPath := filepath.Join(dir, "go.mod")
	if _, err := os.Stat(goModPath); err != nil {
		a.Comment(ins, "git repository does not contain a go.mod file")
		return nil
	}

	mod := goMod{}
	if err := mod.parse(goModPath); err != nil {
		a.Reject(ins, "cannot parse go.mod file").Annotate(
	...
	tags, ok := a.ResponseAnnotation(&git.UploadPackInspector{}, "tags")
	if ok {
	...
	// Reject if depth > 1
	if !isShallow {
		a.Reject(ins, "go module found but repository is not shallow").Annotate(notes)
		return nil
	}

	// Reject if multiple wants
	if len(wants)+len(want_refs) > 1 {
		a.Reject(ins, "go module found with multiple refs").Annotate(notes)
		return nil
	}

	// Reject if version not found
	if a.Metadata.Version == "" {
		a.Reject(ins, "cannot find go module version tag").Annotate(notes)
		return nil
	}

	a.Approve(ins, "go module found").Annotate(notes)
