...
func (s *goModuleGitSuite) TestGoModuleGitInspectArtefact(c *C) {
	for _, tc := range []struct {
		...
		is_shallow  bool
		has_version bool
		opinion     metadata.OpinionKind
		reason      string
	}{
		{goModuleGitFetch, true, true, metadata.Approved, "go module found"},
		{goModuleGitFetch, false, true, metadata.Rejected, "go module found but repository is not shallow"},
		{goModuleGitFetch, true, false, metadata.Rejected, "cannot find go module version tag"},
	} {
		a := metadata.NewArtefact()
