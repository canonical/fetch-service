...
func (ins *AptReleaseInspector) InspectRequest(a *metadata.Artefact) error {
	...
	if info, err := newInReleaseUrlInfo(u); err == nil {
		a.Hold(ins, "valid URL for Release file").Annotate(
			metadata.Annotation{
				"origin":       info.origin,
		...
		if ok {
			a.Hold(ins, "valid URL for packages file").Annotate(notes)
		} else {
			a.Reject(ins, "attempt to download packages file before Release").Annotate(notes)
		}
		...
		_, ok := ins.release[repo]
		if ok {
			a.Hold(ins, "valid URL for translation file").Annotate(notes)
		} else {
			a.Reject(ins, "attempt to download translation file before Release").Annotate(notes)
		}
	}
...
func (ins *AptReleaseInspector) validatePackagesFile(f ReadAtSeeker, a *metadata.Artefact) error {
	...
	if err != nil {
		a.Reject(ins, "invalid path for packages file")
		return nil
	}
...
func (ins *AptReleaseInspector) validateTranslationFile(f ReadAtSeeker, a *metadata.Artefact) error {
	...
	if int64(entry.Size) != a.Metadata.Size {
		a.Reject(ins, "translation file size mismatch").Annotate(
			metadata.Annotation{
