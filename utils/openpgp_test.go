...
type pgputilSuite struct{}

var _ = Suite(&pgputilSuite{})

var (
	key1 = testutils.PGPKeys["key1"]
	key2 = testutils.PGPKeys["key2"]
)
...
var verifyClearSignTests = []verifyClearSignTest{{
	...
	pubKeys:   []*packet.PublicKey{key1.PubKey},
	relerror:  "openpgp: .*invalid signature: RSA verification failure.*",
}, {
...
func (s *pgputilSuite) TestVerifySignature(c *C) {
	for _, test := range verifyClearSignTests {
		c.Logf("Summary: %s", test.summary)

		sigs, body, err := utils.DecodeClearSigned([]byte(test.clearData))
		if err == nil {
			err = utils.VerifyAnySignature(test.pubKeys, sigs, body)
		}
		if test.relerror != "" {
			c.Assert(err, ErrorMatches, test.relerror)
			continue
